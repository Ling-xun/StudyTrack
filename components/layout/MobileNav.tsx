      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "flex h-14 flex-col items-center justify-center gap-1 rounded-lg text-xs font-semibold text-slate-500 transition-[background-color,color,box-shadow] duration-150 ease-out",
              active && "shadow-sm",
            )}
            style={
              active
          ...
          >
            <Icon className="h-5 w-5 transition-colors duration-150" style={active ? { color: "#ccfbf1" } : undefined} aria-hidden="true" />
            {item.label}
          </Link>
        );
      })}
